  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', width: '100%' }}>
      {data.map((item) => {
        ...
        if (item.probability >= 75) color = '#ef4444'; // Very High: Red
        else if (item.probability >= 50) color = '#f97316'; // High: Orange
        else if (item.probability >= 25) color = '#eab308'; // Moderate: Yellow
